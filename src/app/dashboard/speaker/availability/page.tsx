...
import { AvailabilitySlot, RecurrenceRule } from "@/types";
...
function generateSlotsFromRule(
  rule: Pick<RecurrenceRule, "speakerId" | "dayOfWeek" | "hour" | "minute" | "autoConfirm">,
  recurrenceId: string,
  weeks: number
...
function AvailabilityContent() {
  ...
  const handleAddRule = async (e: React.FormEvent) => {
    ...
    const generated = generateSlotsFromRule(
      { speakerId: userProfile.uid, dayOfWeek: ruleDay, hour: h, minute: m, autoConfirm: ruleAutoConfirm },
      ruleRef.id,
      WEEKS_AHEAD
    );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 px-4 py-6">
      ...
      <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
        ...
        <form onSubmit={handleAddRule} className="mb-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto_auto]">
          <select
            value={ruleDay}
